<template>
  <div>
    ...
    >
      <h1 class="h1">
        {{ isNew ? "Tạo dịch vụ" : `Sửa dịch vụ #${postId}` }}
      </h1>
      <div class="row" style="gap: 8px">
        <RouterLink class="btn btn-secondary" to="/service-posts"
          >Quay lại</RouterLink
        >
    ...
    <div class="card" style="margin-top: 16px">
      <div class="grid">
        ...
        <label class="label">
          ...
          <input v-model="form.slug" class="input" placeholder="dich-vu-hay" />
        </label>
      </div>
      ...
      <label class="label">
        ...
        <textarea
          ...
          class="textarea"
          rows="3"
          placeholder="Tóm tắt ngắn cho dịch vụ"
        />
      </label>
...
import type { ServiceCategory, ServicePost, MediaAsset } from "../lib/types";
...
const form = ref({
  ...
  slug: "",
  category_id: 0,
  status: "draft" as ServicePost["status"],
  excerpt: "",
  content: "",
  featured_image: "",
  meta_title: "",
...
function onMediaSelected(assetOrAssets: MediaAsset | MediaAsset[]) {
  ...
  if (mediaPickerTarget.value === "featured") {
    ...
    const tag = `<img src="${asset.url}" alt="service-image" />`;
...
async function loadPost() {
  ...
  const res = await apiFetch<{ post: ServicePost }>(
    `/api/admin/service-posts/${postId.value}`,
  );
  const p = res.post;
...
async function save() {
  ...
  try {
    ...
    if (isNew.value) {
      ...
      router.push(`/service-posts/${res.post.id}`);
    } else {
      await apiFetch(`/api/admin/service-posts/${postId.value}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
    }
    toastSuccess("Đã lưu dịch vụ thành công.");
  } catch (e) {
